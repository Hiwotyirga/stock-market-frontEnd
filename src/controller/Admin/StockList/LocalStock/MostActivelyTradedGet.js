...
function MostActivelyTradedGet() {
  ...
  useEffect(() => {
    const fetchStocks = async () => {
      try {
        const response = await axios.get('http://localhost:8080/most-actively-traded/most-active-trades');
  ...
  const handleEdit = (id) => {
    navigate(`/most-actively-traded/${id}`);
  };

  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(`http://localhost:8080/most-actively-traded/most-active-trades/${id}`, {
        headers: {
      ...
      if (response.status === 200) {
        ...
        setTopGainers(topGainers.filter(stock => stock.id !== id)); // Correctly filter by id
      } else {
    ...
    <div className="container" style={{ fontSize: '15px' }}>
      <main className="main-content">
        <div className="header d-flex justify-content-between align-items-center">
          ...
          <Link to='/activetrade' className="btn btn-primary">Add Stock</Link>
        </div>
